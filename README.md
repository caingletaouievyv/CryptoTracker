...
| Holdings snapshot | ![Holdings snapshot](docs/demo/HoldingsSnapshot.gif) | 1. Sidebar → **Holdings**.<br>2. Enter symbol, quantity, and source; optional sell target and buy zone.<br>3. **Add row** if needed, then **Save holdings**.<br>4. Saved balances are the quantity source for **Portfolio**. |
| Portfolio dashboard | ![Portfolio dashboard](docs/demo/PortfolioDashboard.gif) | 1. Sidebar → **Portfolio**.<br>2. Review the stat row: total value, cost basis, unrealized P/L, BTC dominance.<br>3. Review the holdings table: quantity, price, value, average cost, P/L, allocation %, and strategy tag. |
| Add transaction | ![Add transaction](docs/demo/AddTransaction.gif) | 1. Sidebar → **Transactions**.<br>2. Complete **Add transaction** (symbol, type, quantity, price, fee, date).<br>3. Select **Add**; the ledger refreshes.<br>4. **Portfolio** cost basis updates from buys with price &gt; 0. |
| Import trades (CSV) | ![Import trades (CSV)](docs/demo/ImportTrades(CSV).gif) | 1. Sidebar → **Transactions** → **Import trades** → **CSV file**.<br>2. Choose a `.csv` (template or exchange export).<br>3. Confirm row count → **Import CSV**; zero prices backfill automatically.<br>4. Import updates the ledger only; save **Holdings** separately to change portfolio quantity. |
...
| Theme toggle | ![Theme toggle](docs/demo/ThemeToggle.gif) | 1. Top bar → theme control.<br>2. Dark or light mode applies.<br>3. Preference persists across reload. |
